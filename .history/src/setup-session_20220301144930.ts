...
export const setupSession = async (app) => {
  ...
  app.use(
    session({
      ...
      resave: import { connect } from 'react-redux'
      import React, { Compotruenent } from 'react'
      
      type Props = {}
      
      type State = {}
      
      export class setup-session extends Component<Props, State> {
        state = {}
      
        render() {
          return (
            <div>setup-session</div>
          )
        }
      }
      ...
      export default connect(mapStateToProps, mapDispatchToProps)(setup-session),
